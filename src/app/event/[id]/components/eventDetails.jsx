...
function EventDetails(props) {
  ...
  const [registerUntilDate, setRegisterUntilDate] = useState(
    event.register_until,
  );
  const [registrationLink, setRegistrationLink] = useState(
    event.registration_link,
  );
  const [hostedRegistration, setHostedRegistration] = useState(
    event.hosted_registration,
  );
  const [hostLink, setHostLink] = useState(event.host_link);
  const [daysLeftToRegister, setDaysLeftToRegister] = useState(
    ...
    <>
      <div className="event-cover-details grid grid-cols-2 lg:grid-cols-12 gap-2 ">
        ...
        <div className="col-span-2 lg:col-span-6 ">
          <p className="event-days-left " suppressHydrationWarning>
            {daysLeftToRegister}
          </p>
          <Link
            className={
              (CalculateDaysLeft(registerUntilDate) === "Registration closed"
                ? "disabled-link "
                : null) +
              "col-span-1 inline-flex h-10 items-center justify-center rounded-md bg-primary text-primary-foreground px-8 text-sm font-medium shadow transition-colors hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50"
            }
            href={hostedRegistration ? registrationLink : hostLink}
            target="_blank"
          >
            Register
          </Link>
      ...
      <div className={`event-details-description ${montserratFont.className}`}>
        ...
        <pre className={`event-description ${montserratFont.className}`}>
          {eventDescription}
        </pre>
      </div>
      <div
        className={`event-mode-duration-container ${montserratFont.className}`}
      >
        <EventMode eventMode={eventMode} eventVenue={eventVenue} />
        <EventDuration eventDuration={eventDuration} />
      </div>
      <div className="event-winner-div rounded-lg shadow-sm bg-secondary p-6 gap-6">
        {eventWinners === null ||
        Object.keys(eventWinners).length === 0 ? null : (
          <EventWinners eventWinners={eventWinners} />
        )}
      </div>
      ...
      <div className="event-grid-container grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch justify-stretch">
        ...
        <div
          style={{ width: "100%" }}
          className={`lg:col-span-1 lg:row-span-2 ${montserratFont.className}`}
        >
...
function CalculateEventDuration(totalmins) {
  ...
  if (hourss) {
    if (hourss == 1) {
      duration = duration + hourss + " Hour";
    } else {
      duration = duration + hourss + " Hours";
    }
    if (mins) {
      duration = duration + ", ";
    }
