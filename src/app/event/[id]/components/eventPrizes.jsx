...
export default function EventPrizes(props) {
    ...
    if (props.eventPrizes === null || props.eventPrizes.length === 0) {
        ...
        Object.entries(props.eventPrizes[specialPrizesIndex]).forEach(([key, value]) => {
            specialPrizesArray.push(<div className="event-special-prize-item"><div><SecondPrizeIcon/></div><p className="event-special-prize-title">{key} </p><p className="event-special-prize">&ensp;{value}</p></div>);
        })
    }
        ...
        <>
            <div className="event-prize-container rounded-lg border shadow-sm bg-secondary border-none p-6 gap-6">
                <div className="event-prizes">
                    ...
                    <ul>{props.eventPrizes.map((value, index) => {
                        if (typeof value === 'object') {
                            return null;
                        }
                        if (index === 0) { return <div className="event-prize-item" key={index}><FirstPrizeIcon /> {value}</div> }
                        else if (index === 1) { return <div className="event-prize-item" key={index}><SecondPrizeIcon /> {value}</div> }
                        else if (index === 2) { return <div className="event-prize-item" key={index}><ThirdPrizeIcon /> {value}</div> }
                        else { return <div className="event-prize-item" key={index}><p>{index + 1 + `. `} {value}</p></div> }
                    })}</ul>
                </div>
                <div className="event-special-prize-container bold">
                    <p className="event-requirement-title">{specialPrizes ? "Special Prizes":null}</p>
                    {specialPrizes ? <ul>{specialPrizesArray.map((value,index)=>{return(<div key={index}>{value}</div>)})}</ul>
                    :null}
                </div>
            </div>
        </>
