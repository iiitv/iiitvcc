...
//   const hint = isLC
//     ? "Add this code to your LeetCode Profile → Edit → 'About Me' field, then click Verify. You can revert back the changes after verification."
//     : "Add this code to your Codeforces profile → Edit → 'Organization' field, then click Verify. You can revert back the changes after verification.";
...
//         : undefined;
//       setMessage(serverMsg ?? "Could not refresh rating. Try again after a minute.");
//     }
//   };
...
import { motion, AnimatePresence } from "framer-motion";
import { supabase } from "@/utils/supabase/client";
import { generateVerifyCode } from "../_actions/generate-code";
import { verifyProfile, unlinkProfile } from "../_actions/verify-profile";
import { refreshRating } from "../_actions/refresh-rating";
...
function PlatformSection({
  ...
  // Tick the cooldown down every second once set
  useEffect(() => {
  ...
  // Server sends messages like "Please wait 47s before trying again."
  const startCooldownFromMessage = (msg: string | undefined) => {
  ...
  const handleGenerate = async () => {
    ...
    setStatus("loading");
    setMessage(null);
    const res = await generateVerifyCode(platform, input.trim());
    if (!res.success) {
      setStatus("error");
      setMessage(res.message ?? "Failed to generate code");
      return;
    }
    onProfileUpdate(
      isLC
        ? {
            leetcode_username: input.trim(),
            leetcode_verify_code: res.code,
            leetcode_verified: false,
            leetcode_rating: null,
          }
        : {
            codeforces_username: input.trim(),
            codeforces_verify_code: res.code,
            codeforces_verified: false,
            codeforces_rating: null,
          },
    );
  ...
  const handleVerify = async () => {
    setStatus("loading");
    setMessage(null);
    const res = await verifyProfile(platform);
    if (res.success) {
      onProfileUpdate(
        isLC
          ? { leetcode_verified: true, leetcode_rating: res.rating }
          : { codeforces_verified: true, codeforces_rating: res.rating },
      );
      setStatus("success");
      setMessage(
        res.rating !== null && res.rating !== undefined
          ? `Verified! Your rating: ${res.rating}`
          : "Verified! You appear as Unrated (no contests attended yet).",
      );
    } else {
      setStatus("error");
      setMessage(res.message ?? "Verification failed");
      startCooldownFromMessage(res.message);
    }
  };
  ...
  const handleRefresh = async () => {
    setStatus("loading");
    setMessage(null);
    const res = await refreshRating(platform);
    if (res.success) {
      onProfileUpdate(
        isLC ? { leetcode_rating: res.rating } : { codeforces_rating: res.rating },
      );
      setStatus("success");
      setMessage(
        res.rating !== null && res.rating !== undefined
          ? `Rating updated to ${res.rating}`
          : "Updated — still Unrated.",
      );
    } else {
      setStatus("error");
      setMessage(res.message ?? "Could not refresh rating. Try again later.");
      startCooldownFromMessage(res.message);
    }
  };
  ...
  const handleUnlink = async () => {
    ...
    setStatus("loading");
    setMessage(null);
    const res = await unlinkProfile(platform);
    if (res.success) {
      onProfileUpdate(
        isLC
          ? {
      ...
      setStatus("error");
      setMessage(res.message ?? "Could not unlink. Try again.");
    }
  };
